...
/**
 * Render a same-repository caller, for the repository that owns `ci.yml`.
 *
 * `uses: ./.github/workflows/ci.yml` resolves to the checked-out commit, so
 * this file never needs retagging when the reusable workflow changes.
 */
export function renderLocalCaller(
  detected: DetectedLanguage[],
  options: ManifestOptions,
  version: string,
): string {
...
/**
 * The `with:` block for a caller.
 *
 * Every declared input is passed explicitly, including the ones the workflow
 * also defaults. A same-repository `uses: ./...` call is validated against the
 * callee's declared inputs, and relying on defaults there has proven fragile
 * (the workflow fails to start rather than reporting a missing input), so the
 * caller states the full contract.
 */
function callerInputs(detected: DetectedLanguage[], options: ManifestOptions): string[] {
  const gateScript =
    options.root === '.'
  ...
  return [
    ...
    ...Object.entries(DEFAULT_VERSIONS).map(([input, value]) => `      ${input}: "${value}"`),
  ];
}
